...
- **★ マイルストーンを運用する**（**2026-08-30**。#141）—— **単位は到達点**（成果そのものの
  名前）で、**版名（`v0.1.0`）にしない**。名前は**日本語**。**作るのは、閉じる条件が 1 文で
  書けて、入れる issue が最低 1 本あるとき**（`description` にその 1 文を書く。
  **作成 API は issue を同時に入れられない**ので、**作ったら続けて付ける** —— 規律は
  「0 本で作らない」ではなく「**0 本のまま放置しない**」）。**入れないのは、Dependabot の PR ／ 時間で来る保守
  （依存・CI・脆弱性）／ PR そのもの ／ 過去の closed issue** —— **属さないなら付けない**
  ...
  - **`due_on` は外から決まっている日付があるときだけ。作業量から逆算した締切は置かない**
    ...
    （下の `## 文章の値` の「一文の長さの閾値は置いていない」と同じ判断）
  - **`gh` に `milestone` サブコマンドは無い**（**2026-08-30 実測**。v2.87.3）——
    作成・close・削除は `gh api repos/propagandist/grabado/milestones`。
    **付ける・引くは `gh issue create/edit/list -m` で通る**が、
    **`list -m` は付けた直後に空を返した**（**2026-08-30 実測**。十数秒後は番号・タイトル・
    `--search` の 3 通りとも引けた）——**空で焦って二重に付けない**。
    **`gh api 'repos/{owner}/{repo}/issues?milestone=N'` は即答する**
...
- **★ 版を切ったら Release を出す**（**2026-08-30**。#150）—— **`assets` は付けない**
  （イメージはレジストリで配らない。#83 の決めたこと 5）、**変更を列挙しない**
  ...
  **タグだけでは右柱に何も出ない**。手順は `docs/BRANCHING.md` の「リリース」節
  - **org 正本はリリースノートについて何も決めていない**（§3.7 の守る値は「タグが 1 つ以上
    ある」だけ。`work-conventions.md` §6 の 12 項目にもリリース系は無い）——
