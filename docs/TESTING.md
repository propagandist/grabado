...
| ファイル | 担当 |
|---|---|
| [`../tests/node/live-tree.test.ts`](../tests/node/live-tree.test.ts) | **複数のキーに属する行を消すと、どのキーからも消えること**（#216）・消えた列が保存バイト列に出ないこと・`Table` ごと / `clearTables()` 経由でも残骸が出ないこと ＋ **キーと行の双方向リンクの不変条件**。**FK の伝播がサイクルで止まること**（#212 / #231）と、**非循環では伝播の回数が変わらないこと** ＋ **相互 FK が UI の経路で作れること** ＋ **テーブルのリネームが relation の本数に依らないこと**（#234） |

**★ リネームの多重適用は「本数を引数に取る」形で押さえた**（#234。2026-09-10）。
`Table.setTitle()` の内側ループが `row.relations` を 1 本ずつ回して**同じ行に**置換を掛けており、
**新名が旧名を含むと本数だけ効いた**（`users` → `users_v2` で、relation 2 本なら
`users_v2_v2_id`、3 本なら `users_v2_v2_v2_id`）。テストは 1 / 2 / 3 本の 3 点で見ているので、
**「本数に依らない」こと自体が検査になっている。**

**★ 畳んだあとに意味が読めるかも見ている。** `r.row1 != row` は「**参照される側であるときだけ**
子行名を追随させる」判定で、畳むと真偽 1 つに潰れる。参照される列と参照されない列を
1 つのテーブルに並べ、**後者が据え置かれる**ことを固定した。
...
### 読み込みと保存の関門 — 規則は 1 か所、メッセージは呼び手が持つ（#232 / #233 / #263）

**設計モデルの不変条件**を、**読み込む前**と**書き出す前**の両方で見る。
規則の実体は [`../frontend/js/io/validate.ts`](../frontend/js/io/validate.ts) の 1 か所で、
**メッセージだけ呼び手が書く** —— 保存側は「保存できない」、読み込み側は「開けない」で、
利用者が次に打つ手が違う。

| ファイル | 担当 |
|---|---|
| [`../tests/node/design-guards.test.ts`](../tests/node/design-guards.test.ts) | **キーが存在しない列を指している**（#232）・**同名のテーブル**（#233）・**同名の列**（#263）の 3 つ。どれも**今開いている設計が残る**こと・**メッセージが位置を指す**こと・**読めたものは必ず保存できる**ことを見る ＋ **XML 互換読み込みにも同じ関門が掛かる** |

**★★ 落ちる場所が要件。** `clearTables()` より前で落ちないと、**今開いている設計を消してから**
失敗することになる。だから全部のケースが「今開いている設計が残っている」を見ている。

**★★ 読み込みと保存を揃える。** 片方だけ足すと、**#233 が潰したのと同じ非対称**が新しくできる
（読めるのに保存できない ／ 保存できたのに開けない）。#263 では**画面から作れる**ほうを
テストで踏ませている —— `addRow("id")` は列名の一意性を見ないので、**UI で同名の列は作れる**。

**★ 同名の列は、同名のテーブルより壊れ方が重い**（2026-09-10 実測）:

| | 同名のテーブル（#233） | 同名の列（#263） |
|---|---|---|
| 読み込み | 通っていた（黙って参照先が寄る） | 通っていた（黙って参照先が寄る） |
| 保存 | **拒んでいた** | **通っていた** |
| DDL | —— | **実行できない**（`CREATE TABLE parent (id INTEGER, id TEXT)`） |

**★ テーブルをまたいで見ない。** SQL が禁じるのは 1 つのテーブルの中の重複だけで、
`users.id` と `orders.id` は正常。**その 1 本をテストで固定してある。**
...
**★ 訂正**（**2026-09-10**。#263）—— 元は「**行名の重複は関門が見ていない**ので、
XML の読み込みで実際に到達する」と書いていた。**#263 が関門に足したので、その経路は閉じた。**
**消さずに残す** —— 索引化が「先勝ち」を保っていることの検査は生きており、
**関門を通らない呼び手**（AI パッチの適用と introspection の取り込み）では今も踏める。
テストはその経路へ移した。
...
**★ jsdom で数えられないもの**: `LayoutCount`（jsdom はレイアウトしない）／ **`alignTables()` の
折り返しの結果**（#297 から `#area` の実寸に依存する。jsdom は 0 を返すので**既定値へ落ちる** ——
**回数そのものは jsdom でも数えられる**。計算は純関数で DOM を 1 度も読まない）／ 実時間。**この分担は上の「なぜ 2 系統あるのか」と
同じ形**で、新しい規律を作っていない。
...
### 整列 — DOM を 1 度も触らない層（#297）

**寸法を引数で受けるので、jsdom も実ブラウザも要らない。** 現行 `alignTables()` が Node の
テスト対象外だったのは `offsetWidth` を直に読むためで、**測るのを呼び手へ移した時点で
その矛盾が消えた**。実測では作れない盤面（「幅 5000 が 1 本と幅 100 が 1 本」等）も入力にできる。

| ファイル | 担当 |
|---|---|
| [`../tests/node/layout.test.ts`](../tests/node/layout.test.ts) | [`../frontend/js/io/layout/layered.ts`](../frontend/js/io/layout/layered.ts) を直に叩く（ハーネス不要・**jsdom も不要**）。**不変条件 7 種**（決定論 / 順序保存 / 箱が重ならない / 負を出さず grid の倍数 / 成分の bbox が交わらない / **FK の x 区間が交わらない** / 冪等）・**異常系 11 ケース**・旧実装との**相対比較 3 入力** |
| [`../tests/support/layout-metrics.ts`](../tests/support/layout-metrics.ts) | 交差数・線長総和・占有面積の物差し。**出荷コードに入れない。純関数とは別実装で数える** —— 同じ関数を使うと、バグが実装と検査の両方へ同時に写る |

**golden を 1 本も持たない。** 整列後の座標を焼くとフォント・viewport・OS 依存が入る
（#236 と同じ判断）。**絶対値も焼かない** —— 「x が 250 になること」を書くと、層間の空きを
60 から 80 に変えただけで全部赤くなり、**何が壊れたのか読めなくなる**。

**★★ 検査に穴が開きかけた。** 不変条件 6（FK の x 区間が交わらない）は、**同じ層へ落ちた
後退辺**（相互 FK・サイクル）を除く必要がある。素直に「x が同じペアは飛ばす」と書くと、
**層化を殺して全部を層 0 に置く変異が 1 つも検査されずに通る**（全ペアが「同じ x」になる）。
そこで**飛ばした本数を数え、サイクルの無い入力では 0 本であること**を要求している。
**この 1 行で変異 1 が 11 テストを赤にする。**

**変異で確かめた**（2026-09-13）—— 層化を殺すと **11 テスト**が赤、量子化を外すと **12 テスト**が赤。

**★ 実ブラウザ側は `canvas.spec.ts` の「幾何の棚」に `describe("整列")` を 1 つ**（#297 の PR 2）。
**新しい spec を作らない。採取側（`tests/support/geometry.ts`）に 1 行も足さない** —— 見たいものは
既に採れている（実測の `left/top/width/height`・モデルの `x/y`・`designer` の寸法・端点）。

| 見るもの | なぜ実ブラウザでしか見られないか |
|---|---|
| 実測寸法で箱が 1 対も重ならない | 純関数は**渡された寸法の上では**重ならないと保証するが、**その寸法が実際の DOM と一致しているか**はここでしか分からない |
| **2 回続けて整列しても座標が 1 つも動かない** | **shrink-to-fit を捕まえるのはこの 1 本だけ**（下） |
| 端点がアンカーに乗り続ける | 既存の `expectEndpointsOnAnchors()` を再利用する。**新しい判定を書かない** |
| 盤面寸法が全テーブルを含む | `sync()` を通っていることの確認 |
| **選択中のテーブルがあっても上が全部成り立つ** | 選択中は縁も中心も 1px ずれる（`relation.ts` の分岐） |

**★★ shrink-to-fit の罠**: `js/table.ts` が「`left` によって shrink-to-fit の上限（`#area` の
右端まで）が変わりうる」と宣言している。**右端に届く位置へ置くと、置いた後の実測幅が置く前と
違い、2 回目の整列が 1 回目と別の結果を出す**。対策は折り返し幅を `#area` 実寸 − 余白に抑えること
（`js/io/layout/apply.ts` の `wrapWidthFor()`。**CSS の 3000 を焼かない**）で、
**検知はこの「2 回で 1px も動かない」だけ**。
...
| # | 画面 | 見るもの |
|---|---|---|
| 1 | ツールバー | **41 ボタンにアイコンが出ている**（#170。★ **2026-09-14 に 42 から 1 つ減った** —— #306 で整列ボタンを外したため。**区切り線が二重に出ていないこと**も見る）。**ラベルが切れていない** —— de / ru は元から 1 個切れており、溝を空けて 3 個 / 5 個になった（#170 の実測）。`text-overflow: ellipsis` が付いているので、**切れていれば「…」が見える** |
