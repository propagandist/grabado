 ...
 *   テストは恒等関数を渡せる。
 *
 * ★ **「保存はされない」と必ず書く。** 正本は git 管理のファイルで save するまで変わらない。
 *   それを知らせないと、**戻れることに気づけない**。
 *
 *   **★ 訂正（2026-09-12。#290）—— 元は「grabado に undo は無く、気に入らなければ保存せず
 *   読み直せば戻ることが undo の代わりになっている」だった。消さずに残す。**
 *   #288 / #289 で undo が入り、**戻り方が 1 つから 2 つになった** —— 直前の適用は
 *   **undo 1 手**、保存した状態までは**読み直し**。**書く理由のほうは変わらない。**
 *
 * ★ **「丸ごと」と書く。** rejections があるので**実際に当たった件数は選んだ件数以下**で、
 *   N 手にすると「何回 Ctrl+Z を押せばいいか」が画面から読めない。適用 1 回 = undo 1 手
 *   （js/io.ts の aiapply が 1 回だけ commit する）であることを、この 1 枚が言い切る。
 */
