...
#
# ★ **GHCR で配ると決めた**（**2026-09-04**。issue #164）—— `ghcr.io/propagandist/grabado`。
#   段階2-0 の決めたこと 5 は「配れない」ではなく「したくなった時点で別 issue」だったので、
#   **予約どおり別 issue で決めた**。**実体（OCI ラベル ／ push-by-digest → manifest ／ SBOM）は
#   issue #165 が入れる** —— このファイルにラベルが 1 つも無いのは、その前だからである。
#   **手で build する経路は残す**（README の「起動」）。
