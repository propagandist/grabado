...
export function parseDesignJson(
     ...
     * typeIndex / fkTypeFor の古いキャッシュを新パレットに当てる既知の癖（4-0b で
     * 意図的に温存）を JSON 経路にも持ち込むことになる。cookie の db は変わらないので
     * リロードで元に戻る半端な状態も作る。現行 UI も「db の変更にはリロードが要る」
     * という契約を明示している（locale の optionsnotice）ので、そちらに揃える。
     *
