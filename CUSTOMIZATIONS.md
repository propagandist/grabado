...
| ワークフロー | 所要 | 内訳 |
|---|---:|---|
| `ci-frontend` | **85 秒** | **Chromium の取得 39（46%）** / vitest 11 / 実ブラウザ 12 / dist 5 / typecheck 3 / known-issues 3 / npm ci 3 |
| `ci-server` | **107 秒** | `./gradlew build` 93 |
| `ci-image`（新設） | **131 秒** | **イメージ build 78** / Chromium 24 / 起動 6 / 13 本 11 |
...
予告していた数字を採った —— **85 秒のうち 39 秒**。`ci-image` も同じものを 24 秒払っている。
...
**★ paths が効くこと（起動契機の検出試験）は、この PR では確かめられなかった** ——
`pull_request` の `paths` は **PR 全体の差分**で評価されるので、**ワークフロー自身を足した PR は
常に起動する**。**次の文書だけの PR で観測する**（下の申し送り）。
...
- **★ paths の検出試験が残っている** —— **次の文書だけの PR で、3 本とも起動しないことを見る**
  （この PR では確かめられない理由は上）
