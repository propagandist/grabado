...
8. **フロント描画エンジンは今回作り直さない**（Tier 2）。**★ UI framework 移行は 2026-09-05 に検討して不採用**（#167。**全面も部分も採らない**）—— **元は「将来判断・要確認」**で、**禁止ではなかったので判断に足る材料を採った**。決定打は**壊れる本数ではなく安全網の穴の位置**。根拠・却下した案・**再考の条件 3 つ**は `CUSTOMIZATIONS.md` の同日（**規約の中身をここへ写さない**）。**時間では引き直さない。**
...
- **★ マイルストーンを運用する**（**2026-08-30**。#141）。名前は**日本語**。
  ...
  **ここへ写さない。** ここが持つのは**値**だけ:
  ...
  - **★★ 版に紐づかない到達点は、例外として版名なしで作ってよい**（**2026-09-04**。#187。
    **ユーザー判断**）—— **下の 2 つを両方満たすときだけ**:
    - **基準 1: 成果の本体が git の外にある** —— **版を切っても配られない**
      （Railway ／ Porkbun ／ 証明書 ／ 外部サービスの設定）
    - **基準 2: 版の順序と独立している** —— **いつ閉じても他の版に影響しない**。
      **版名なしは順序を持たない**ので、**順序が要らないものだけ**が例外になれる
    - **片方しか満たさないなら版に紐づける。迷ったら版に入れる**（例外は狭いほうに倒す）
    - **タイトルは素の到達点名**（`公開デモを立てる` の形）。**`運用: 〜` のような第 2 の型を
      作らない** —— **型が 2 つになると、作るたびに「どちらの型か」の判断が発生する**
    - **`description` の規律は同じに掛かる**（**何が成立したら閉じるかを 1 文**）。
      **例外だから緩めるのではない**
    - **★ 代償は「一覧の型が揃わないこと」。それでよいと決めた** ——
      **org で版と到達点が混在しているのは grabado だけ**（**2026-09-04 実測**。archive を除く
      33 本のうち運用は 11 本で、**版でないマイルストーンを持つのは grabado 1 本と `fantasma`
      4 本だけ**。`fantasma` は **4 本とも到達点**で、**版のマイルストーンを 1 本も持たない**
      —— Release もタグも 0 本だから）。**基準 2 がこの代償を吸収している** ——
      **例外は順序を持たないものに限られる**ので、**版の並びは読めるまま残る**
    - **★ #1 `公開デモを立てる` はリネームしない**（**タイトルも `description` も触らない**）——
      **v0.1.0 のリリースノートが「この版に入っていないもの: 公開デモ（grabado.dev）は
      まだ立っていない」と明記している**ので、**`v0.1.0 — 公開デモを立てる` にすると嘘になる**。
      **closed のマイルストーンを後から書き換えない**（closed の issue を再 open して直さないのと
      同じ判断。#144）。理由は `CUSTOMIZATIONS.md` の 2026-09-04
...
- **★ 版を切ったら Release を出す**（**2026-08-30**。#150）—— **`assets` は付けない**
  ...
  **タグだけでは右柱に何も出ない**。手順は `docs/BRANCHING.md` の「リリース」節
  ...
  - **★★ ノートの中身の値**（**2026-09-05**）—— **読者の正本は org `repo-surface-baseline.md`**
    （§1 が「**日本語話者の採用候補者・取引先**。利用拡大は目的ではない」、§3.7 の★が
    「**Release を読む人が org の外にいる**」）。**ここが持つのは、そこから出る値だけ**:
    - **★ 読者が 1 人増えたのは 2026-09-04**（#164 で分類 P になった日）—— **配布物を
      受け取った人**。**v0.1.0 のときは居なかった**ので、**v0.1.0 の型をそのまま次に使わない**
    - **見出しの主語を読者側にする** —— **目次だけで「自分が何をすべきか」が決まる**形にする。
      「こちらが何をしたか」の物語にしない
    - **書かない** —— org の分類語 ／ org 正本の節番号 ／ ブランチ運用 ／ CI の内部設計 ／
      **自分たちへの規律の宣言**（「変更を列挙しない」は**規律であって、読者への情報ではない**）
    - **その版単体で完結させる** —— **冒頭 2 行で「grabado が何か」**を書く。
      **Release ページに来た人は README を開かない**
    - **「効かない範囲」を書く** —— **機能ではなく境界を書く**（到達しない脆弱性は「しない」と、
      既定で動かない機能は「動かない」と）
    - **本文の見本は v0.2.0 のノート**（org `work-conventions.md` §2 の「**書式は実物を 1 本
      指す**」と同じ形）
    - **なぜ要るか**: **この値が無かったので、v0.2.0 の初稿が「org の審査者」に向いた** ——
      分類語と org 規約の充足報告と CI の内部設計が**本文の 3 割**を占め、**利用者に向いた
      見出しは 5 本中 1 本**だった。org `writing-baseline.md` §7 の「**前提の省略** —— 調べた
      直後に書くと、調べて分かったことが書く側では既に前提へ変わっている」そのもの。
      **判断は `CUSTOMIZATIONS.md` の 2026-09-05**
