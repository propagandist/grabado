...
> **元の記述は消さない。** §3.7 Releases の守る値は「**タグが 1 つ以上ある**」で、
> **この棚卸しの時点で `git tag -l` は 0 本**だった（同日の #141 が実測している）。
> **§3 を数えたときに §3.7 だけ落ちた。**
>
> **なぜ落ちたか** —— §3.1〜§3.6 は**リポジトリの中身とAbout 欄**（README・ライセンス・
> description・topics）で、**手元と `gh api repos/...` で 1 度に見える**。
> **§3.7 だけが git の側**（タグ）にあり、**同じ確かめ方の並びに乗っていなかった**。
> **「§3 を満たしているか」を 1 つの手順で見ようとすると、ここが構造的に漏れる。**
>
> **タグは 2026-08-30 に #145 で打ち**（`v0.1.0` → `5a649e2`）、
