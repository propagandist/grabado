...
| **CI 3 本の `paths`** | **`CLAUDE.md` も `CUSTOMIZATIONS.md` も、`ci-frontend` / `ci-server` / `ci-image` のどれにも無い** —— **この PR ではジョブが 1 本も起動しないのが正常**（「全ジョブ緑」ではない） |
...
- **この PR で CI 3 本が本当に起動しないことを確かめる** —— `paths` を読んだ結果であって、
  **実走の観測ではない**
