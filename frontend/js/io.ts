...
export class IO {
     ...
     *   変わらない。それを [applyNotice] が毎回書く。
     *
     *   **★ 訂正（2026-09-12。#290）—— 元は「grabado に undo は無いが、気に入らなければ
     *   保存せず読み直せば戻る」だった。消さずに残す。** #288 / #289 で undo が入り、
     *   **戻り方が 2 つになった** —— ここが末尾で `historyManager.commit()` を 1 回だけ
     *   呼ぶので、**何件当たっても undo 1 手で丸ごと戻る**。
     *
