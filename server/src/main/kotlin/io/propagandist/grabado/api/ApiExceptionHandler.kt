...
import io.propagandist.grabado.introspect.IntrospectionFailedException
import io.propagandist.grabado.introspect.UnknownSourceException
...
class ApiExceptionHandler {
    ...
    /** `keyword` が受け取れない形（[io.propagandist.grabado.design.NameRejection]）。body は返さない。 */
     ...
     *
     * ★ **429 は §5 の語彙に無い status だった。** 増やす PR で `check()` と `locale` を同じ PR で
     * 広げるのが上の規律だが、**11-2a はフロントが `/api/ai/review` を 1 度も呼ばず、この status が
     * フロントに届く経路が無かった**（5-1b で 400 を足したときと同じ形）。**11-3 の配線と同時に
     * 広げた**ので、いまは `check()` が `case 429` を、locale が `http429` を持っている。
     *
