...
- **`Closes` / `Fixes` は、既定が `main` だった間は発火しなかった**（**2026-08-23 実測**。
  コミット本文と PR 本文の両方に書いても効かず、issue は手で閉じた。#73）——
  GitHub がキーワードで閉じるのは**既定ブランチへのマージ**のときだけなので、
  **既定がマージ先と一致した 2026-08-26 以降は発火する見込み**。**#95 のマージで観測し、
  結果をここへ訂正する**（それまでは閉じたかを毎回見る）
