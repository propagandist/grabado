...
省略したパレットは従来どおり**（初期列は `id` 1 列 ＋ autoincrement、既定型は添字 0）。

**段階6-8d で 8 プロファイルすべてが持つ**（6-4 の `postgresql` に始まり、6-7a 〜 6-8d で
残る 7 本）。テンプレートは「そのプロファイルが house 既定を最も近く表す形」なので、
パレットの現代化と一体でなければ決められなかった。省略できる規定が残っているのは、
**旧 XML 同梱の `<datatypes>`**（段階4-2b 以前の設計ファイル）を読む経路のため。
...
**段階6-8d で 8 プロファイルすべてがこの規則になった** —— 6-8c までは未現代化側に
...
**★ #233 で読み込み側にも同じ関門が入った**（2026-09-09）。4-4 の時点では**保存だけ**を拒んで
いたので、**「読めて、黙って別の設計になり、保存だけができない」行き止まり**が残っていた ——
利用者から見ると「開いたのに保存できない」で、なぜ設計が変わったのかは画面のどこにも出ない。
規則の実体は [`../frontend/js/io/validate.ts`](../frontend/js/io/validate.ts) の 1 か所で、
保存側と読み込み側がそれぞれのメッセージを持つ（次に打つ手が違うため）。
...
**読めた設計は保存できる。** 保存側が拒むものは読み込み側も拒む —— 非対称そのものが不具合だった。

- **同名のテーブルが 2 つ以上ある**（#233）
- **キーが、そのテーブルに無い列を指している**（#232）

**★ 落ちる場所が要件**: `clearTables()` より **前**。`Designer.fromJson` は「壊れた JSON で
例外が出たときに今開いている設計を消さない」ために parse を clear の前に置いており、
その不変条件はこの検査にも掛かる。

**★ XML 互換読み込みにも同じ関門が掛かる。** 同梱パレットを持たない XML は parse が clear より
前なのでそのまま守れる。**同梱パレットを持つ経路だけは clear が先**で「今の設計を消さない」は
守れないが（[`ARCHITECTURE.md`](ARCHITECTURE.md) §5 の順序制約）、**拒みはする** ——
通すと、relation が黙って別のテーブルに繋がった状態や、キーに存在しない列が載った状態が
ライブツリーに入り、保存もできない行き止まりになる。

**★ #232 は「黙って捨てる」だった。** キーが存在しない列を指していても例外にならず、
`Table.findNamedRow` が返す `false` を `Key.addRow` が静かに握りつぶしていた（`(false).owner` は
`undefined` で TypeError にならない）。**読み込みは通り、保存すると欠けた状態が正本に
書き戻される** —— 落ちるより悪い。
