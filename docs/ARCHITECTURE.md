...
### 5.7 描画の費用モデル（#206。2026-09-09 実測）

**読み込みの費用は、数えられるものすべてが N の 1 次式に乗る。**

| 数えるもの | 式 | 導出 |
|---|---|---|
| `Row.update` | `9N - 1` | **列の総数**（合成設計は 1 テーブル 9 列で、先頭だけ FK が無く 8 列） |
| `Row.redraw` | `11N - 2` | 列の総数 ＋ **キー登録数**（PRIMARY が N 本、INDEX が N-1 本、各 1 列） |
| `Relation.redraw` | `N - 1` | 関係の本数 |
| `Table.redraw` | `23N - 3` | 実測から当てはめた（内訳は未分解） |
| `offsetWidth` / `offsetHeight` | `48N - 8` | 同上。**2 つは常に同数**（ペアで読まれている） |
| `offsetTop` | `4N - 4` | 同上 |
| `offsetLeft` | `2N - 2` | 同上 |
| DOM ノード | `61N + 67`（jsdom） | 67 は設計と無関係な UI の分 |

**★★ #207 で 2 つの数が動いた**（2026-09-10 実測。**上の表は 2026-09-09 の観測として残す**）。

| | #206（2026-09-09） | #207 の後（2026-09-10） | なぜ |
|---|---|---|---|
| `offsetWidth` / `offsetHeight` | `48N - 8` | **`25N - 5`** | `Table.redraw()` の 2 回読みを 1 回にした。**差はちょうど `tableRedraw`（`23N - 3`）** ——導出できる差で、当てはめた値ではない |
| `LayoutCount`（N=300） | 9,598 | **7,198** | relation を 2 パス（全部読んでから全部書く）にした |
| `offsetW+H`（N=300） | 28,784 | **14,990** | 上の 1 行目 |
| 読み込み ms（N=300） | 6,178 | **7,222 〜 10,260**（3 回の幅） | **判定しない。** 同じ機械・同じ回数で 1.4 倍ぶれる |

**★ 実時間はまだ動かない。** 落ちたのは「読み出しの回数」と「レイアウトが走る回数」で、
**回数そのものが N×C に比例して積み上がる部分**（`Table.redraw()` が読み込み中に
`N(1 + 2C + P) + 2N` 回呼ばれる）は #210 の仕事。**#207 は 1 回あたりの費用を下げただけ**で、
上の「実効 O(N²)」の構造は変えていない。

**★ 式の正本は [`../tests/support/synthetic.ts`](../tests/support/synthetic.ts) の `SCALE_COUNTS`。**
元は `tests/node/scale.test.ts` と `tests/scale/load.spec.ts` が同じ式を別々に持っており、
**#207 で片方だけ直り、もう片方が赤くなって気づいた**（2026-09-10）。

**★★ 二次項は「回数」には無い。それでも実時間は超線形になる。**

| N | 読み込み ms | `offsetW+H` | `LayoutCount` | 比 |
|---|---|---|---|---|
| 10 | 60 | 944 | 328 | 0.35 |
| 50 | 376 | 4,784 | 1,647 | 0.34 |
| 100 | 798 | 9,584 | 3,297 | 0.34 |
| 300 | **6,178** | 28,784 | 9,598 | 0.33 |

**レイアウトが実際に走る回数も線形**（読み出し 3 回につきおよそ 1 回）。にもかかわらず
実時間は 10 → 100 で **17.5 倍**、100 → 300 で **6.6 倍**になる。

> **レイアウト 1 回のコストが DOM のサイズに比例する。**
> 回数 O(N) × 1 回のコスト O(N) = **実効 O(N²)**。

これが #206 の「食い違い 2 つ」への答えになる:

- **食い違い 1**（初回読込に関係の二次項が出るか）—— **出ない。** `applyDesignModel()` が
  「全テーブル生成 → リレーション接続」の順なので、テーブル生成中は `relations` が空で
  `Table.redraw()` 内のループが 0 回で抜ける。`Relation.redraw` はちょうど `N - 1` 回
- **食い違い 2**（フラッシュ回数が O(N*C) なら実効は二乗）—— **フラッシュ回数は O(N)**
  （O(N*C) ではない）。**それでも実効は二乗**で、原因は回数ではなく**1 回のコスト**

**★★ #210 で実時間が動いた**（2026-09-10 実測。**上の 2 つの表は当時の観測として残す**）。

| N=300 | #206（09-09） | #207 の後 | **#210 の後** |
|---|---|---|---|
| 読み込み ms | 6,178 | 7,222 〜 10,260 | **1,467 / 1,554 / 1,646 / 2,204**（4 回） |
| `offsetW+H` | 28,784 | 14,990 | **2,996** |
| `LayoutCount` | 9,598 | 7,198 | **1,201** |
| `Layout` / 読み出し | 0.33 | 0.48 | **0.40** |

**★ 落ちたのは「回数」で、比は動いていない。** レイアウト 1 回のコストが DOM のサイズに
比例する構造（上の「実効 O(N²)」）は**そのまま**で、**掛ける回数を N×C から 3N に落とした**。

読み込み中のカウンタ（jsdom。式の正本は
[`../tests/support/synthetic.ts`](../tests/support/synthetic.ts) の `SCALE_COUNTS`）:

| 数えるもの | #206 | #207 | **#210** | 導出 |
|---|---|---|---|---|
| `offsetWidth` / `offsetHeight` | `48N - 8` | `25N - 5` | **`5N - 2`** | 実際に描いた `Table.redraw()` が `3N` 回（各 1 回読む）＋ `Relation.measure()` が `N-1` 本 × 2 回 |
| `Table.redraw()`（**実際に描いた**） | `23N - 3` | `23N - 3` | **`3N`** | `resumeRedraw()` の `N` ＋ ff hack の `2N` |
| `Table.redraw()`（呼び出し） | `23N - 3` | `23N - 3` | `24N - 3` | 早期 return で戻った分も入る |

**★ 「呼び出し」と「実際に描いた」を分けて数えている**（#210 で `tableRedrawWorked` を足した）
—— 抑止は `Table.redraw()` の先頭の早期 return なので、**呼び出し回数では効果が測れない**。

**★ 重いのは読み込みだけ。** 書き出しも 1 操作も桁が違う:
...
書き出しはライブツリーを歩いて文字列を組むだけで **DOM を 1 度も触らない**。
`sync()` は `minSize` を読むだけで、`offsetWidth` を 1 回も読まない。

**★ FK の型伝播の費用は「鎖の深さ」で決まり、テーブル数では決まらない。**
合成設計は `t(i).parent_id -> t(i-1).id` の形なので**深さ 1**（`t1.parent_id` を指すものが
無い）。深い鎖を持つ設計では N 段まで伸びうる —— **別の軸なので、別の合成設計で測る**。

**★★ 整列の費用**（#297。**2026-09-13 実測**）—— **回数が 2 実行系で完全に一致した。**
...
**★ 係数を 1 つも当てはめていない。** 上の読み込みの表は `offsetTop` / `offsetLeft` /
`tableRedraw` が「実測から当てはめた」ままだが、**整列は全部導出できる** ——
[`../frontend/js/io/layout/apply.ts`](../frontend/js/io/layout/apply.ts) が
**測る / 計算 / 書く の 3 段**に分かれているので、**どの段が何回読むかが構造から決まる**。
...
**★★ 折り返しの結果は実行系で違うのに、回数は 1 つも違わない。** `#area` の実寸に依存するのは
**折り返し幅だけ**で、jsdom では既定値へ落ちる（座標は変わる）。**計算が純関数で DOM を
1 度も読まない**ので、読み出しの回数は実行系に依らない。**この一致自体が、3 段の分離が
効いていることの証拠**である。

**★ 計算区間の DOM 読み出しは 0 回**（[`../tests/node/scale.test.ts`](../tests/node/scale.test.ts)）。
純関数が DOM を import していないことは目で読めば分かるが、**目で読むのは検査ではない** ——
計装したまま `layoutLayered()` を呼び、**全カウンタが 0 のまま**であることを見ている。

**★ 実時間は超線形。判定しない**（記録のみ）—— N が 30 倍で **109 倍**。読み込みと同じ構造で、
**レイアウト 1 回のコストが DOM のサイズに比例する**。**回数は線形**なので、落とすとしたら
1 回あたりのコストの側（#210 の旗で `Relation.measure()` を 2E → E に落とす案は**測ってから判断する**）。
