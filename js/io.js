/*
 * grabado: ES クラス化した（HANDOVER §3 段階3-3）。prototype メソッド 35 本をクラス本体へ
 * 移しただけで、代入と呼び出しの順序は現行のまま（変換は機械的に行い、空白を無視した diff が
 * シグネチャ行と閉じ括弧だけであることを確認している）。
 * this.saveresponse = this.saveresponse.bind(this) 以下 4 行は「プロトタイプのメソッドを
 * インスタンスの own property で上書きする」現行の形を温存している（OZ.Request に
 * コールバックとして渡すため）。
 */
class IO {
    constructor(owner) {
    ...
    showDropboxAuthenticate(connectedCallBack) {
    ...
    saveresponse(data, code) {
        this.owner.window.hideThrobber();
        this.check(code);
    }

    loadresponse(data, code) {
        this.owner.window.hideThrobber();
        if (!this.check(code)) {
            return;
        }
