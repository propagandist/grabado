...
describe("compose.yaml の書き方（段階2-3 の判断を固定する）", () => {
    ...
    // ★ 2026-09-04（issue #164）にテスト名を変えた。守る値は 1 バイトも変えていない。
    //   元は「レジストリのイメージを指さない（各自が build する）」で、**段階2-0 の決めたこと 5**
    //   が理由だった。GHCR で配ると決めた（#164）あとも `build:` のままにするのは、
    //   **compose を使うのはリポジトリを clone した人**で、**見たいのは手元の変更が入った
    //   イメージ**だからである。**理由が変わってテストが残る**形なので、名前を実態に寄せた。
    test("compose は手元のソースから build する（配布イメージに差し替えない）", () => {
