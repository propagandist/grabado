...
**適用は保存ではない。** 気に入らなければ **`Ctrl+Z` 1 回**で適用の前に戻り、
**保存せずに読み直せば**最後に保存した状態まで戻る。
...
- **使った総額を数える仕組みは無い。** サーバが持つのは上限（レート制限・入力サイズ）で、**予算ではない**
  - **費用は Anthropic の Console で見る** —— grabado 用のワークスペースを作り、そこで作ったキーを渡せば、
    **その分だけが Usage / Cost に出て**、ワークスペースの **Spend limits で月の上限と通知**を置ける
    （ワークスペースは組織の機能）
...
**例外は読み取り専用モードだけ**で、そのときは `-v` が要らない（下の「読み取り専用」）。
...
**このモードでは mount が要らない。** 保存しないので書き先も要らない ——
**1 行で、読むだけのビューアが立つ**（サーバ側の設計は 0 件になる。編集はブラウザの中で完結する）:
