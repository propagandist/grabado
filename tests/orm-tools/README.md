...
`tests/golden/orm/` の 56 本を、**実物の JPA (Kotlin) / JPA (Java) / Prisma / Drizzle が受け付けるか**確かめる。
...
| 種類 | 決め方 |
|---|---|
| **出力が 0 バイト**（`empty` は 4 本とも該当） | **走査で自動判定。書かない** ——「空なら道具に渡すものが無い」は規則であって一覧ではない |
...
- **★ 道具の版のうち本体にあるもの（typescript / kotlin）は本体から読む**（#251）——
  写していた時期に、Dependabot が本体の typescript だけを 5.9.3 -> 7.0.2 に上げ、
  **写しは 12 日ずれたまま緑だった**（この層は手元でしか回らないので、回した日まで見えない）
- **1 本目が落ちても止めない** ——「残り 2 本が分からない」状態を作らない
- **★ Java だけ golden をファイルへ分けてから渡す**（2026-09-15）—— **javac は 1 つの
  コンパイル単位に public なクラスを 1 つしか許さない**のに、`@IdClass` の id クラスも
  public を要求されるので、1 つの golden に public クラスが複数入る。**golden は 1 バイトも
  変えず**、生成器が出した区切りのマーカーで切って一時ディレクトリへ書く（Prisma の prelude と
  同じ立場）。**マーカーの正本は生成器**（`frontend/js/io/orm/jpa-java.ts` の `FILE_MARKER`）で、
  ここに同じ形を書かない —— **分割が成立することは `npm test` 側でも押さえている**
  （マーカーと `public class` が 1 対 1 で、名前まで一致すること）
- **★ `javac` には `-encoding UTF-8` が要る**（同）—— **既定がプラットフォーム依存**で、
  コンテナのロケールが POSIX だと `顧客` / `氏名` を含む golden を復号できない。
  **kotlinc は既定が UTF-8** なので 1 本目では踏まなかった
- **★ 作業ディレクトリは `cases.ts` の型が持つ**（同）—— 以前は `verify.ts` が
  `target === "jpa" ? "/tmp" : "/work"` と 2 分岐で持っており、**足したターゲットが黙って
  `/work` 側に落ちた**。同じ日に、`script()` の末尾が fallback だったせいで
  **知らないターゲットが Kotlin として走る**ことも直している
